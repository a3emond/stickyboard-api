    ...
    public class MessageRepository : RepositoryBase<Message>
    {
        ...
        public override async Task<Guid> CreateAsync(Message e, CancellationToken ct)
        {
            ...
            await using var cmd = new NpgsqlCommand(@"
                INSERT INTO messages (sender_id, receiver_id, subject, body, type, related_board, related_org, status)
                VALUES (@s, @r, @sub, @b, @t, @rb, @ro, @st)
                RETURNING id", conn);
        ...
        public override async Task<bool> UpdateAsync(Message e, CancellationToken ct)
        {
            await using var conn = await OpenAsync(ct);
            await using var cmd = new NpgsqlCommand(@"
                UPDATE messages SET status=@st WHERE id=@id", conn);
        ...
        {
            await using var conn = await OpenAsync(ct);
            await using var cmd = new NpgsqlCommand("SELECT COUNT(*) FROM messages WHERE receiver_id=@r AND status='unread'", conn);
            cmd.Parameters.AddWithValue("r", userId);
