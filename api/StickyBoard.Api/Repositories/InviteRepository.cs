    ...
    public class InviteRepository : RepositoryBase<Invite>
    {
        ...
        public override async Task<Guid> CreateAsync(Invite e, CancellationToken ct)
        {
            ...
            await using var cmd = new NpgsqlCommand(@"
                INSERT INTO invites (sender_id, email, board_id, org_id, role, token, accepted, expires_at)
                VALUES (@s, @em, @b, @o, @r, @t, @a, @x)
                RETURNING id", conn);
            ...
            cmd.Parameters.AddWithValue("o", (object?)e.OrganizationId ?? DBNull.Value);
            cmd.Parameters.AddWithValue("r", (object?)e.Role ?? DBNull.Value);
        ...
        public async Task<IEnumerable<Invite>> GetPendingForEmailAsync(string email, CancellationToken ct)
        {
            ...
            await using var cmd = new NpgsqlCommand(@"
                SELECT * FROM invites
                WHERE LOWER(email)=LOWER(@em) AND accepted=false AND expires_at>now()", conn);
        ...
        public async Task<IEnumerable<Invite>> GetPendingBySenderAsync(Guid senderId, CancellationToken ct)
        {
            ...
            await using var cmd = new NpgsqlCommand(@"
                SELECT * FROM invites
                WHERE sender_id=@s AND accepted=false AND expires_at>now()
